...
function createNetwork(layerSizes, numClasses = 2, numSamples = 10, eta = -0.10) {
    ...
    function backwardPass(n, l) {
        ...
        for (let k = outK - 1; k >= 1; k--) {
            ...
            for (let j = 0; j <= currSize; j++) {   // j=0 es el bias
                let sum = 0;
        ...
        for (let k = 0; k < numWeightMatrices; k++) {
            ...
            for (let i = 0; i < dstSize; i++) {        // neurona destino (1-indexed en delta)
                const di = delta[k + 1][i + 1];
                for (let j = 0; j <= srcSize; j++) {   // neurona origen (con bias en j=0)
...
if (require.main === module) {
    ...
    console.log('═══════════════════════════════════════════');
    {
        const net = createNetwork([30, 20, 2], 2, 10, -0.10);
        ...
        let r = net.train({ maxEpochs: 5000, targetSSE: 0.01, trainClass: 0, logEvery: 500 });
    ...
    console.log('═══════════════════════════════════════════');
    {
        const net = createNetwork([30, 16, 8, 4, 2], 2, 10, -0.10);
        net.loadPatterns(0, makePatterns(10, 30));
        net.loadPatterns(1, makePatterns(10, 30));
        net.loadLabels(0, [1, 0]);
        net.loadLabels(1, [0, 1]);
        ...
        let r = net.train({ maxEpochs: 10000, targetSSE: 0.01, trainClass: 1, logEvery: 1000 });
        console.log(`Fin: epoch=${r.epoch}  sse=${r.sse.toFixed(6)}\n`);
